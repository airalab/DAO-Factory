...
![Factory for Ethereum Developer](/img/DAO-Factory-Ethereum-Developer-1.jpg)
...
### Взаимодействие пользователя с умным контрактом Ethereum разработчика

![Factory for Ethereum Developer](/img/DAO-Factory-Ethereum-Dev-2.jpg)

Найдя описание контракта разработчика, пользователь обращается к реестру сборщиков DAO Factory для того, чтобы получить информацию об адресе контракта и abi к нему. Пользователь отправляет транзакцию к сборщику, указав необходимые для репликации данные и указав достаточное количество принимаемых токенов (Transaction 1), что приводит к созданию 3 внутренних транзакций (Internal transaction). Первая внутренняя транзакция производит перевод полученной от пользователя оплаты / авторского отчисления на счёт разработчика (Internal Transaction to Smart contact developer EOA). После этого сборщик обращается к библиотеки создателю контракта (Internal transaction 2) с подготовленными данными для репликации контракта. И последней внутренней третьей транзакцией (Internal transaction 3) библиотека-создатель выполняет репликацию (создание копии умного контракта) с новым адресом в сети Ethereum и отправляет событие с данными нового контракта.  
...
![Factory for Ethereum users](/img/DAO-Factory-Ethereum-Dev-1.jpg)
...
![Factory for DApp developers](/img/DAO-Factory-DApp-1.jpg)
